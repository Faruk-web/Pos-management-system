...
class SliderController extends Controller
{
    ...
    public function SliderView()
    {
        ...
        $sliders = Slider::with('ecommerce')->latest()->get();
              return view('backend.slider.slider_all', compact('sliders','ecom'));
    } // end method
    ...
    public function SliderStore(Request $request, $role)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($request->slider_img) {
                ...
                $slider = new Slider;
                $slider->ecom_id = $request->input('ecom_name');
    ...
    public function SliderUpdate(Request $request, $role, $id)
    {
        ...
        if ($request->file('slider_img')) {
            ...
            Image::make($image)->resize(933, 236)->save('upload/slider/' . $name_gen);
            $save_url = 'upload/slider/' . $name_gen;

            $slider_update =  Slider::findOrFail($id)->update([
                'ecom_id' => $request->ecom_name,
                'title' => $request->title,
